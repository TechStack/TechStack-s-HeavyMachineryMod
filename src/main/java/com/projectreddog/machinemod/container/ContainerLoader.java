...
public class ContainerLoader extends Container {
	...
	public ContainerLoader(PlayerInventory inventoryPlayer, EntityLoader loader) {
		this.loader = loader;

		for (int i = 0; i < 1; i++) {
			for (int j = 0; j < 9; j++) {
				addSlotToContainer(new SlotItemHandler(loader.inventory, j + i * 9, 8 + j * 18, 18 + i * 18));
			}
		}
