...
public class ContainerTractor extends Container {
	...
	public ContainerTractor(PlayerInventory inventoryPlayer, EntityTractor tractor) {
		this.tractor = tractor;

		for (int i = 0; i < 6; i++) {
			for (int j = 0; j < 9; j++) {
				addSlotToContainer(new SlotItemHandler(tractor.inventory, j + i * 9, 8 + j * 18, 18 + i * 18));
			}
		}
	...
	public boolean canInteractWith(PlayerEntity player) {
		return tractor.isUsableByPlayer(player);
	}
