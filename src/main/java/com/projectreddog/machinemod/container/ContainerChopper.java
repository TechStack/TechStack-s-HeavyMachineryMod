...
public class ContainerChopper extends Container {

	protected EntityChopper chopper;

	public ContainerChopper(InventoryPlayer inventoryPlayer, EntityChopper chopper) {
		this.chopper = chopper;

		for (int i = 0; i < 1; i++) {
			for (int j = 0; j < 9; j++) {
				addSlotToContainer(new SlotItemHandler(chopper.inventory, j + i * 9, 8 + j * 18, 18 + i * 18));
			}
		}
	...
	public boolean canInteractWith(PlayerEntity player) {
		return chopper.isUsableByPlayer(player);
	}
