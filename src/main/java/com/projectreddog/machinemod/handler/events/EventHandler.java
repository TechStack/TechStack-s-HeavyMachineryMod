...
public class EventHandler {
	...
	public void HarvestDropEvent(BlockEvent.HarvestDropsEvent event) {
		if (event.getState().getBlock() == Blocks.TALLGRASS) {
			...
			if (BiomeDictionary.isBiomeOfType(event.getWorld().getBiomeGenForCoords(event.getPos()), Type.PLAINS)) {
				Random r = new Random();
				if (r.nextFloat() > .8) {
					event.getDrops().add(new ItemStack(ModItems.cornseed));
				}
			} else if (BiomeDictionary.isBiomeOfType(event.getWorld().getBiomeGenForCoords(event.getPos()), Type.SAVANNA)) {
				Random r = new Random();
				if (r.nextFloat() > .97) {
					event.getDrops().add(new ItemStack(ModItems.cornseed));
				}
