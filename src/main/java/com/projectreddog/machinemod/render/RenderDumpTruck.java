...
public class RenderDumpTruck extends Render {
	...
	public void doRender(Entity entity, double x, double y, double z,
		...
		((ModelDumpTruck) this.modelDumpTruck)
				.renderGroupObject("Truck_Base_Cube.002");
		GL11.glTranslatef(0f, -1.1f, 2.8f);
		GL11.glRotatef(((EntityDumpTruck) entity).Attribute1, 1, 0, 0);
		((ModelDumpTruck) this.modelDumpTruck)
				.renderGroupObject("Bed_Cube.000");
		...
		for (int i = 0; i < eDT.getSizeInventory(); i++) {
			...
			if (is != null) {
				...
				IBakedModel ibakedmodel = itemRenderer.getItemModelMesher()
						.getItemModel(is);
				...
				if (ibakedmodel.isBuiltInRenderer()) {
					...
					worldrenderer
							.setVertexFormat(DefaultVertexFormats.field_176599_b);
					...
					for (int k = 0; k < j; ++k) {
						...
						this.RenderHelper_a(worldrenderer,
								ibakedmodel.func_177551_a(enumfacing), -1, is);
					}

					this.RenderHelper_a(worldrenderer,
							ibakedmodel.func_177550_a(), -1, is);
					tessellator.draw();
				}
				GL11.glRotatef(-45, 1, 1, 0);
				even = !even;
	...
	private void RenderHelper_B(WorldRenderer p_175033_1_,
			BakedQuad p_175033_2_, int p_175033_3_) {
	...
	private void RenderHelper_C(WorldRenderer p_175038_1_, BakedQuad p_175038_2_) {
		...
		p_175038_1_.func_178975_e((float) vec3i.getX(), (float) vec3i.getY(),
				(float) vec3i.getZ());
	}

	private void RenderHelper_a(WorldRenderer p_175032_1_, List p_175032_2_,
			int p_175032_3_, ItemStack p_175032_4_) {
		boolean flag = p_175032_3_ == -1 && p_175032_4_ != null;
		BakedQuad bakedquad;
		int j;

		for (Iterator iterator = p_175032_2_.iterator(); iterator.hasNext(); this
				.RenderHelper_B(p_175032_1_, bakedquad, j)) {
			bakedquad = (BakedQuad) iterator.next();
			j = p_175032_3_;

			if (flag && bakedquad.func_178212_b()) {
				j = p_175032_4_.getItem().getColorFromItemStack(p_175032_4_,
						bakedquad.func_178211_c());
	...
	protected ResourceLocation getEntityTexture(Entity entity) {
		return new ResourceLocation("machinemod",
				Reference.MODEL_DUMPTRUCK_TEXTURE_LOCATION);
	}
