...
public class TileEntityFermenterRenderer extends TileEntityRenderer {

	private ModelFermenter teModel = new ModelFermenter();
	...
	public void render(TileEntity tileentity, double x, double y, double z, float f, int i, float a) {
		...
		Direction ef;
		if (tileentity.getWorld().getBlockState(tileentity.getPos()).getBlock() == ModBlocks.machinefermenter) {
			ef = (Direction) tileentity.getWorld().getBlockState(tileentity.getPos()).getValue(BlockMachineModFermenter.FACING);
		} else {
			ef = Direction.NORTH;
		}
		switch (ef) {
	...
	public static ResourceLocation getResourceLocation() {
		if (resourceLocation == null) {
			resourceLocation = new ResourceLocation("machinemod", Reference.MODEL_FERMENTER_TEXTURE_LOCATION);
		}
