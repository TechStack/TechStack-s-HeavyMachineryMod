...
public class MachineModMessageEntityToClientHandler implements
		IMessageHandler<MachineModMessageEntityToClient, IMessage> {

	@Override
	public IMessage onMessage(MachineModMessageEntityToClient message,
			MessageContext ctx) {
		// LogHelper.info("in machineModMessageEntityToClient Handler");
		// LogHelper.info("Message data" + message);
		if (message != null) {
			if (Minecraft.getMinecraft().theWorld != null) {
				Entity entity = Minecraft.getMinecraft().theWorld
